...
if st.button("분석 실행"):
    for symbol in etfs:
        try:
            ...
            st.markdown(f"🔍 **SMA20/50/200:** {sma20} / {sma50} / {sma200}")
            st.markdown(f"📉 **볼린저밴드 상단/하단:** {boll_upper} / {boll_lower}")

            prompt = strategy_prompt(symbol, rsi_val, macd_desc)
            st.markdown(f"💬 **전략 문장 복사용:** `{prompt}`")

            if st.button(f"{symbol} 전략 확인하기"):
                with st.spinner("GPT 전략을 불러오는 중..."):
            ...
            ax.set_title(f"{symbol} RSI 차트", color="white")
